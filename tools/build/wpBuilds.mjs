 ...
 */
export const mkConfig = function () {
  const outputPath = path.resolve('./dist'); // relative to exe dir
  const indexPath = path.resolve('./src/index.html'); // relative to exe dir
  const port = parseInt(process.env.PORT ?? 3000);
  const isDevBuild = process.env.NODE_ENV === 'development';

  let entry; // relative to exe dir, and must be absolute path
  if (fs.existsSync('./src/index.js')) {
    entry = path.resolve('./src/index.js');
  } else if (fs.existsSync('./src/index.tsx')) {
    entry = path.resolve('./src/index.tsx');
  } else {
    throw new Error('Failed to find index.js or index.tsx entry point');
  }
  ...
  return {
    ...
    module: {
      rules: [
        ...
        {
          // allow /node_modules/ because anything TS that we need must be transpiled
          test: /\.([cm]?ts|tsx)$/,
          use: {
            loader: 'ts-loader',
            options: {
    ...
    devServer: {
      ...
      static: {
        directory: path.resolve('./static'), // relative to exe dir
        publicPath: '/static/',
        watch: true,
      },
      host: 'localhost',
